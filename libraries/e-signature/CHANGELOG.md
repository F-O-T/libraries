...
### Changed
- Bump `@f-o-t/crypto` to `^1.3.1` — gains BER indefinite-length and constructed OCTET STRING support, enabling signing with Lacuna Software test certificates and other legacy PFX files
...
### Fixed
- QR code top edge now aligns with the text top edge instead of floating at the bottom or center of the stamp

## [1.7.4] - 2026-03-05

### Fixed
- QR code is now vertically centered within the signature stamp instead of bottom-aligned
...
### Fixed
- `detectSigningPosition` fallback (no signals) now centers the stamp horizontally instead of hardcoding `x: 50`
