...
- `generateTSConfig` no longer excludes plugin directories from the generated `tsconfig.json` — plugin source files are now included in the single main `tsc` declaration pass, which correctly emits `.d.ts` files for all entry points (including `src/plugins/<name>/index.ts`) into `dist/` without spurious side-effect declarations in `src/`
