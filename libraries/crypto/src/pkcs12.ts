import { decodeDer, encodeDer, type Asn1Node } from "@f-o-t/asn1";
import { bytesToOid } from "@f-o-t/asn1";
import { createHash, createDecipheriv, createHmac, pbkdf2Sync } from "node:crypto";
...
export function parsePkcs12(
  ...
  const safeContents = expectSequence(
    authenticatedSafe,
    "AuthenticatedSafe",
  );
...
function verifyMac(
  ...
  const hashLen = hashAlg === "sha1" ? 20 : hashAlg === "sha256" ? 32 : hashAlg === "sha384" ? 48 : 64;
  ...
  if (computedDigest.length !== expectedDigest.length) {
    throw new Pkcs12Error("PKCS#12 MAC verification failed (wrong password?)");
  }
  let diff = 0;
...
function decryptEncryptedData(
  ...
  if (encryptedContentNode.class === "context" && encryptedContentNode.tag === 0) {
    if (encryptedContentNode.constructed && Array.isArray(encryptedContentNode.value)) {
...
function decryptShroudedKeyBag(
  ...
  const decryptedPkcs8 = decryptPbe(encryptedData, algOid, algParams, password);
...
function readOid(node: Asn1Node): string {
  if (node.tag !== 6 || node.constructed) {
    throw new Pkcs12Error(
      `Expected OID, got tag 0x${node.tag.toString(16)}`,
    );
  }
