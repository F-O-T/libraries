...
describe("formatFromBigInt", () => {
  ...
  describe("trailing zeros", () => {
    ...
    test("trims trailing zeros with explicit true", () => {
      expect(formatFromBigInt({ value: 12300n, scale: 2, trimTrailingZeros: true })).toBe("123");
    });

    test("preserves trailing zeros when requested", () => {
      expect(formatFromBigInt({ value: 12300n, scale: 2, trimTrailingZeros: false })).toBe("123.00");
    });
    ...
    test("preserves significant trailing zeros", () => {
      expect(formatFromBigInt({ value: 12340n, scale: 2, trimTrailingZeros: false })).toBe("123.40");
    });
    ...
    test("preserves all decimal zeros when requested", () => {
      expect(formatFromBigInt({ value: 100000n, scale: 5, trimTrailingZeros: false })).toBe("1.00000");
    });
  });
  ...
  describe("padding and precision", () => {
    ...
    test("pads with leading zeros and preserves trailing zeros", () => {
      expect(formatFromBigInt({ value: 1n, scale: 5, trimTrailingZeros: false })).toBe("0.00001");
    });

    test("handles large scale", () => {
      expect(formatFromBigInt({ value: 12300000000n, scale: 10 })).toBe("1.23");
    });

    test("handles very small values", () => {
      expect(formatFromBigInt({ value: 1n, scale: 10 })).toBe("0.0000000001");
    });
  ...
  describe("edge cases", () => {
    test("handles very large numbers", () => {
      expect(formatFromBigInt({ value: 99999999999999999999n, scale: 2 })).toBe("999999999999999999.99");
    });
    ...
    test("handles zero with trimTrailingZeros false", () => {
      expect(formatFromBigInt({ value: 0n, scale: 2, trimTrailingZeros: false })).toBe("0.00");
    });
  });
