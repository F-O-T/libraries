...
import { bankersRound, convertScale, roundDown, roundUp } from "../src/round";
...
describe("convertScale", () => {
   describe("scaling up (increasing precision)", () => {
      test("multiplies by power of 10", () => {
         expect(convertScale({ value: 123n, fromScale: 2, toScale: 4 })).toBe(
            12300n,
         );
         expect(convertScale({ value: 500n, fromScale: 1, toScale: 3 })).toBe(
            50000n,
         );
      });

      test("handles zero scale to positive scale", () => {
         expect(convertScale({ value: 42n, fromScale: 0, toScale: 2 })).toBe(
            4200n,
         );
      });
   ...
   describe("scaling down (decreasing precision)", () => {
      test("truncates by default", () => {
         expect(convertScale({ value: 12345n, fromScale: 4, toScale: 2 })).toBe(
            123n,
         );
         expect(convertScale({ value: 12399n, fromScale: 4, toScale: 2 })).toBe(
            123n,
         );
      });
   ...
   describe("same scale (no-op)", () => {
      test("returns value unchanged", () => {
         expect(convertScale({ value: 12345n, fromScale: 4, toScale: 4 })).toBe(
            12345n,
         );
         expect(convertScale({ value: -999n, fromScale: 2, toScale: 2 })).toBe(
            -999n,
         );
      });
   });
   ...
   describe("edge cases", () => {
      ...
      test("handles large scale differences", () => {
         expect(convertScale({ value: 1n, fromScale: 0, toScale: 10 })).toBe(
            10000000000n,
         );
         expect(
            convertScale({
