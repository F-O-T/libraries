...
describe("parseToBigInt", () => {
  ...
  describe("scale handling", () => {
    ...
    test("handles scale 0 (integers)", () => {
      expect(parseToBigInt({ value: "123.45", scale: 0, roundingMode: "truncate" })).toBe(123n);
    });
  ...
  describe("rounding modes - truncate", () => {
    test("truncates excess decimals (positive)", () => {
      expect(parseToBigInt({ value: "123.456", scale: 2, roundingMode: "truncate" })).toBe(12345n);
    });
  ...
  describe("rounding modes - round (banker's rounding)", () => {
    test("rounds half to even (positive, tie to even)", () => {
      expect(parseToBigInt({ value: "12.5", scale: 0, roundingMode: "round" })).toBe(12n);
    });

    test("rounds half to even (positive, tie to odd)", () => {
      expect(parseToBigInt({ value: "13.5", scale: 0, roundingMode: "round" })).toBe(14n);
    });
    ...
    test("rounds up when > 0.5", () => {
      expect(parseToBigInt({ value: "12.6", scale: 0, roundingMode: "round" })).toBe(13n);
    });

    test("rounds down when < 0.5", () => {
      expect(parseToBigInt({ value: "12.4", scale: 0, roundingMode: "round" })).toBe(12n);
    });
  });

  describe("rounding modes - ceil", () => {
    test("rounds up (positive)", () => {
      expect(parseToBigInt({ value: "123.001", scale: 2, roundingMode: "ceil" })).toBe(12301n);
    });

    test("rounds toward zero (negative)", () => {
      expect(parseToBigInt({ value: "-123.001", scale: 2, roundingMode: "ceil" })).toBe(-12300n);
    });

    test("no rounding when exact", () => {
      expect(parseToBigInt({ value: "123.45", scale: 2, roundingMode: "ceil" })).toBe(12345n);
    });
  });

  describe("rounding modes - floor", () => {
    test("rounds down (positive)", () => {
      expect(parseToBigInt({ value: "123.999", scale: 2, roundingMode: "floor" })).toBe(12399n);
    });

    test("rounds away from zero (negative)", () => {
      expect(parseToBigInt({ value: "-123.001", scale: 2, roundingMode: "floor" })).toBe(-12301n);
    });

    test("no rounding when exact", () => {
      expect(parseToBigInt({ value: "123.45", scale: 2, roundingMode: "floor" })).toBe(12345n);
    });
  });

  describe("edge cases", () => {
    test("handles very large numbers", () => {
      expect(parseToBigInt({ value: "999999999999999999.99", scale: 2 })).toBe(99999999999999999999n);
    });
  ...
  describe("validation errors", () => {
    ...
    test("throws on non-finite number", () => {
      expect(() => parseToBigInt({ value: Number.POSITIVE_INFINITY, scale: 2 })).toThrow();
    });
