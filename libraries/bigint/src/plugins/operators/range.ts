...
import { compare, isZero, isPositive, isNegative } from "../../compare";
...
export const bigintBetweenOperator = createOperator({
  ...
  evaluate: (actual: unknown, expected: unknown): boolean => {
    ...
    return (
      compare({ a: value.value, b: range.min.value, scale: value.scale }) >= 0 &&
      compare({ a: value.value, b: range.max.value, scale: value.scale }) <= 0
    );
  },
...
export const bigintIsPositiveOperator = createOperator({
  ...
  evaluate: (actual: unknown, expected: unknown): boolean => {
    ...
    const valueIsPositive = isPositive({ a: value.value, b: 0n, scale: value.scale });
...
export const bigintIsNegativeOperator = createOperator({
  ...
  evaluate: (actual: unknown, expected: unknown): boolean => {
    ...
    const valueIsNegative = isNegative({ a: value.value, b: 0n, scale: value.scale });
