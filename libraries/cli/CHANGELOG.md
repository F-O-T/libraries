...
- Add `module` condition to all generated `exports` entries in `syncPackageJsonExports` so Vite/rolldown resolving under conditions `["module", "browser", "development", "import"]` can locate ESM entry points — previously `module` was absent, causing a "not exported under the conditions" error for plugin sub-paths (e.g. `./plugins/react`)
